...
async def add_shipment_w1_callback(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer('Выберите артикул для изменения поставок:', reply_markup=await kb.inline_incomes())


@router.callback_query(F.data.startswith('article'))
async def increse_incomes_callback(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await callback.message.answer(f'Введите увелечение для {callback.message.text}')
    await state.set_state(Reg.increase_incomes)


@router.message(Reg.increase_incomes)
async def increase_incomes(message: Message, state: FSMContext):
    try:
        increase_value = int(message.text)
        if 0 <= increase_value:
            config['Article']['F.data'] = increase_value
            with open('config.json', 'w') as config_json:
                json.dump(config, config_json, indent=4)
            await message.answer(f'Увелечение для {F.data} = {increase_value}', reply_markup=kb.main)
            await state.clear()
        else:
